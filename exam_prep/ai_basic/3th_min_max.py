...
def minmax_manual_scale(df):
    ...
    for col in numeric_cols:
        ...
        if range_val == 0:
            ...
            scaled_df[col] = (scaled_df[col] - min_val) / range_val
...
def test():
    ...
    assert np.isclose(scaled_test_df['A'].min(), 0.0) and np.isclose(scaled_test_df['A'].max(), 1.0)
    assert np.isclose(scaled_test_df['B'].min(), 0.0) and np.isclose(scaled_test_df['B'].max(), 1.0)
